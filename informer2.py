...
class myDataLoader:
    def __init__(self, batch_size):
        ...
        train_set = EcgDataset(train_data, y_train_encoded)
        val_set = EcgDataset(test_data, y_test_encoded)
...
class PostionalEncoding(nn.Module):
    def __init__(self, dropout=0.1, max_seq_len=5000, d_model=128, batch_first=False):
        super().__init__()
...
class ClassificationHead(nn.Module):
    def __init__(self, n_classes=6):
        ...
        self.seq = nn.Sequential(
            nn.Linear(feature_dim, 256),  # Use the feature_dim instead of input_size
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.ReLU(),
...
class ProbSparseSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = torch.bernoulli(torch.full((batch_size, self.n_head, seq_len, seq_len), 0.5, device=self.device)).bool()
...
class Informer(nn.Module):
    def __init__(self, d_model, n_head, ffn_hidden, n_layers, drop_prob, feature_dim, device):
        super(Informer, self).__init__()
        self.device = device
        self.encoder_input_layer = nn.Linear(feature_dim, d_model)
        self.encoder = Encoder(d_model, ffn_hidden, n_head, n_layers, drop_prob, details=False, device=device)
        self.class_head = ClassificationHead(n_classes=6)  # No input_size specified
...
model = Informer(d_model=d_model, n_head=n_head, ffn_hidden=ffn_hidden,feature_dim = feature_dim, n_layers=n_layer, drop_prob=drop_prob,device=device).to(device)
